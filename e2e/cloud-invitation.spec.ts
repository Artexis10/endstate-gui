...
interface CaptureMockOptions {
  hostedBackup?: Record<string, unknown>;
}

async function installCaptureMock(page: Page, options: CaptureMockOptions = {}) {
  const hostedBackup = options.hostedBackup ?? {
    supported: true,
      ...
      runEndstateStreaming: async (
        ...
        if (command === 'backup') {
          return {
            ...
            envelope: {
              success: true,
              data: { signedIn: false, issuerUrl: 'https://cloud.example.test/' },
            },
            ndjsonEvents: [],
          };
        }
...
test.describe('post-capture cloud invitation', () => {
  ...
  test('offers Endstate Cloud once, then never again after a reload', async ({ page, baseURL }) => {
    ...
    await expect(invitation).toContainText('Keep an encrypted version with Endstate Cloud');
  ...
  test('fails closed when the engine reports a self-hosted provider', async ({ page, baseURL }) => {
    await installCaptureMock(page, {
      hostedBackup: {
        supported: true,
  ...
  test('fails closed when the engine omits its provider discriminator', async ({ page, baseURL }) => {
    await installCaptureMock(page, {
      hostedBackup: {
        supported: true,
        issuerUrl: 'https://cloud.example.test/',
