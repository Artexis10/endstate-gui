...
The GUI is a commercial product (€39 lifetime, 3 device activations). It needs a license gate that blocks unlicensed users from the app while supporting offline use and machine-bound activation via checkout provider.

## What Changes

- Rust backend module (`license.rs`) for machine fingerprinting (MachineGuid + hostname + InstallDate → SHA-256), checkout provider API integration (activate/validate/deactivate), and disk cache at `%APPDATA%/Endstate/license.json` with offline grace
...
- `src-tauri/src/license.rs` — New: machine fingerprint, checkout provider API calls, cache persistence, `LicenseStatus`/`LicenseCache` types, 6 unit tests
