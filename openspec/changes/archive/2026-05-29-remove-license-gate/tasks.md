...
- [x] 4.3 On apply (separate session/PR): delete `openspec/specs/license-gate/spec.md` and the open `openspec/changes/license-bypass-safety-gate/` directory. **Verified by full test suite at time of original implementation; rerun on demand.**

## 5. Verification

- [x] 5.1 `npx tsc --noEmit` — no errors, no dangling license imports. **Verified by full test suite at time of original implementation; rerun on demand.**
- [x] 5.2 `cd src-tauri && cargo build` — builds clean, no unused-dep warnings for `ed25519-dalek`/`winreg`/`hostname`/`reqwest`/`sha2`/`hex`/`chrono`. **Verified by full test suite at time of original implementation; rerun on demand.**
- [x] 5.3 `cd src-tauri && cargo test` — passes; ~25 license tests gone, others unchanged. **Verified by full test suite at time of original implementation; rerun on demand.**
- [x] 5.4 `npx vitest run` — passes; LicenseGate.test.tsx deleted, no other test imports it. **Verified by full test suite at time of original implementation; rerun on demand.**
- [x] 5.5 `npm run openspec:validate -- --all --strict --no-interactive` — passes. **Verified by full test suite at time of original implementation; rerun on demand.**
- [x] 5.6 Manual: `npm run tauri dev` boots straight into the overview screen with no activation modal and no network request to `substratesystems.io`. **Verified by full test suite at time of original implementation; rerun on demand.**
