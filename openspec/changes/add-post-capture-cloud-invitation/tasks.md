...
- [x] Pure `isCloudInvitationEligible` covering: not previously shown, not dismissed, positive managed-provider availability, no prior managed-account evidence, no current managed sign-in at any subscription status, and auto-backup consent neither open nor pending
...
**File**: `src/components/app/intent/save-flow.tsx`
- [x] `onCloudInvitationShown` fired synchronously in the durable save-success path, ahead of every state update that could render the card
- [x] A failed namespaced settings write returns `false` and leaves the invitation hidden rather than treating React state as durable consumption
...
- [x] Exact copy, no price, accessible close, Escape, and outside-pointer dismissal
...
### Task 6: App wiring
**File**: `src/App.tsx`
- [x] Persisted flags plus managed-provider eligibility and prior-account evidence passed to `SaveFlow`
- [x] `autoBackupConsentPending` derived from the dialog's open state plus the same condition that opens it; managed-account evidence is recorded after a positive managed sign-in
...
### Task 7: Tests
- [x] `src/components/app/intent/save-flow-cloud-invitation.test.tsx`: eligibility, record-before-present ordering, persistence failure, all dismissal paths, restart/crash, copy, count, availability, self-host, and prior-account pinning
