...
- Keep installed/static protocol registration as the primary path, then call
  the deep-link plugin's explicit `register("endstate")` call during Windows and Linux startup as a
  best-effort self-heal for missing installer or desktop integration state.
