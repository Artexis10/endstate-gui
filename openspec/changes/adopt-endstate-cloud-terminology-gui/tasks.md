...
- [x] `hosted-backup-signed-out.tsx`: heading and description renamed without a hard-coded price literal
...
- [x] Description rewritten to describe the fresh local capture created after detected drift
- [x] Behaviour unchanged; the public setting is named `Scheduled setup checks`
...
- [ ] `npm run test:e2e`
- [ ] Repo-wide grep for `Hosted Backup` / `hosted backup` with every survivor classified as a retained identifier, a contract-document citation, or historical text
