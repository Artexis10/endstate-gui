...
The settings page SHALL render an Account section when `capabilities.features.hostedBackup.supported === true` AND the user is signed in. The section SHALL display the user's email (read-only), the current subscription status, a Manage subscription link, and a Sign out button.
