 ...
 *   - default form shows email + "Have a Hosted Backup claim code?" link
...
describe('SignUpForm — default branch', () => {
  it('renders email + the claim-code toggle link', () => {
    ...
    expect(
      screen.getByRole('button', { name: /have a hosted backup claim code/i }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('textbox', { name: /claim code/i }),
...
describe('SignUpForm — claim mode toggle', () => {
  it('hides email and reveals the paste field; preserves password', async () => {
    ...
    expect(screen.getByRole('textbox', { name: /claim code/i })).toBeInTheDocument();
    expect(screen.getByLabelText('Password')).toHaveValue(VALID_PASSWORD);
    expect(
      screen.getByRole('button', { name: /claim account/i }),
    ).toBeInTheDocument();
  });
  ...
  it('returns to default form when "Use a regular sign-up instead" is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: /use a regular sign-up instead/i }),
    );
  ...
  it('keeps submit disabled while the pasted token is malformed', async () => {
    ...
    await user.type(
      screen.getByRole('textbox', { name: /claim code/i }),
      'not-a-token',
    );
    await user.type(screen.getByLabelText('Password'), VALID_PASSWORD);
    await user.type(screen.getByLabelText('Confirm password'), VALID_PASSWORD);

    expect(screen.getByRole('button', { name: /claim account/i })).toBeDisabled();
...
describe('SignUpForm — claim submission', () => {
  it('calls backupClaim with the normalised token from a pasted deep link', async () => {
    ...
    await user.type(screen.getByLabelText('Password'), VALID_PASSWORD);
    await user.type(screen.getByLabelText('Confirm password'), VALID_PASSWORD);
    await user.click(screen.getByRole('button', { name: /claim account/i }));
  ...
  it('surfaces CLAIM_TOKEN_INVALID via friendlyAuthError, with no CTA', async () => {
    ...
    await user.type(screen.getByRole('textbox', { name: /claim code/i }), VALID_TOKEN);
    await user.type(screen.getByLabelText('Password'), VALID_PASSWORD);
    await user.type(screen.getByLabelText('Confirm password'), VALID_PASSWORD);
    await user.click(screen.getByRole('button', { name: /claim account/i }));
