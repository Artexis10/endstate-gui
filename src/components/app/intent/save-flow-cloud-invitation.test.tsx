...
describe('SaveFlow — post-capture cloud invitation', () => {
  describe('eligibility', () => {
    ...
    it('does not present it to an active subscriber', async () => {
      renderSaveFlow({
    ...
    it.each([
      ['signed in without an active subscription', { hostedBackupSupported: true, hostedBackupSignedIn: true, hostedBackupSubscriptionStatus: 'grace' as const }],
      ['signed out with hosted backup supported', { hostedBackupSupported: true, hostedBackupSignedIn: false, hostedBackupSubscriptionStatus: undefined }],
      ['engine without hosted backup support', { hostedBackupSupported: false, hostedBackupSignedIn: true, hostedBackupSubscriptionStatus: 'active' as const }],
    ])('presents it when the subscriber gate is false — %s', async (_label, props) => {
      renderSaveFlow(props);
      await scanAndSave();
  ...
  describe('dismissal is permanent', () => {
    it('retires the invitation when the user keeps it local', async () => {
      const onCloudInvitationDismissed = vi.fn();
    ...
    it('retires the invitation and routes to the hosted-backup pane on the primary action', async () => {
      ...
      fireEvent.click(screen.getByRole('button', { name: 'Protect with Endstate Cloud' }));
  ...
  describe('presentation', () => {
    ...
    it('states the saved-locally outcome and the offer without a price', async () => {
      ...
      expect(
        screen.getByText('Keep future versions protected automatically'),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          'Endstate Cloud keeps end-to-end encrypted versions of this setup ready if this PC or drive fails.',
        ),
      ).toBeInTheDocument();
